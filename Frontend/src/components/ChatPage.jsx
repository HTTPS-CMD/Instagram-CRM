...
import {getChatMessages, getChatRooms, sendChatMessage} from '../api';
            ...
            <Box sx={{
                ...
                <Box p={2} pb={1}>
                    <TextField
                        ...
                        InputProps={{
                            startAdornment: <InputAdornment position="start"><SearchIcon fontSize="small"
                                                                                         sx={{color: theme.palette.text.secondary}}/></InputAdornment>,
                            style: {
                                color: theme.palette.text.primary,
                        ...
                        <List>
                            {groupRooms.length > 0 && (<>
                                <Typography variant="caption" fontWeight="bold" sx={{
                                    px: 2, py: 1, display: 'block', color: theme.palette.text.secondary
                                }}>گروه‌ها</Typography>
                                {groupRooms.map((room) => (<ListItemButton
                                    key={room.id}
                                    onClick={() => setSelectedRoom(room)}
                                    selected={selectedRoom?.id === room.id}
                                    sx={{
                                        px: 2, py: 1.5, '&.Mui-selected': {
                                ...
                                >
                                    ...
                                    }}><GroupIcon/></Avatar></ListItemAvatar>
                                    <ListItemText
                                        primary={<Typography fontWeight="bold" noWrap
                                                             color="text.primary">{room.name.replace('گروه پروژه: ', '')}</Typography>}
                                        secondary={<Typography variant="caption" color="text.secondary"
                                                               noWrap>{room.last_message ? room.last_message.text : '...'}</Typography>}
                                    />
                                </ListItemButton>))}
                            </>)}
                            {pvRooms.length > 0 && (<>
                                <Typography variant="caption" fontWeight="bold" sx={{
                                    px: 2, py: 1, display: 'block', mt: 1, color: theme.palette.text.secondary
                                }}>شخصی</Typography>
                                {pvRooms.map((room) => (<ListItemButton
                                    key={room.id}
                                    onClick={() => setSelectedRoom(room)}
                                    selected={selectedRoom?.id === room.id}
                                    sx={{
                                ...
                                >
                                    <ListItemAvatar><Avatar src={room.avatar} sx={{
                                        bgcolor: alpha(theme.palette.info.main, 0.2), color: theme.palette.info.main
                                    }}><PersonIcon/></Avatar></ListItemAvatar>
                                    <ListItemText
                                        primary={<Typography fontWeight="bold" noWrap
                                                             color="text.primary">{room.name}</Typography>}
                                        secondary={<Typography variant="caption" color="text.secondary"
                                                               noWrap>{room.last_message ? room.last_message.text : '...'}</Typography>}
                                    />
                                </ListItemButton>))}
                ...
                {selectedRoom ? (<>
                    <Box sx={{
                        ...
                        <Stack direction="row" alignItems="center" spacing={2}>
                            <IconButton size="small" onClick={() => setSelectedRoom(null)} sx={{
                                display: {md: 'none'}, color: theme.palette.text.primary
                            }}><SearchIcon sx={{transform: 'rotate(180deg)'}}/></IconButton>
                            <Avatar
                                sx={{bgcolor: selectedRoom.type === 'group' ? 'warning.main' : 'primary.main'}}>{selectedRoom.type === 'group' ?
                                <GroupIcon/> : <PersonIcon/>}</Avatar>
                            <Box>
                                <Typography variant="subtitle1" fontWeight="bold"
                                            color="text.primary">{selectedRoom.name.replace('گروه پروژه: ', '')}</Typography>
                                <Typography variant="caption"
                                            color="text.secondary">{selectedRoom.type === 'group' ? 'گروه پروژه' : 'آنلاین'}</Typography>
                            </Box>
                        </Stack>
                    ...
                    <Box sx={{
                        flexGrow: 1, overflowY: 'auto', p: 3, display: 'flex', flexDirection: 'column', gap: 2
                    }}>
                        {loadingMessages ? <Box display="flex" justifyContent="center"
                                                mt={5}><CircularProgress/></Box> : messages.map((msg) => {
                                ...
                                <Paper sx={bubbleSx(isMe)}>
                                    ...
                                    {msg.reply_to_text && (<Box sx={{
                                        ...
                                        <Typography variant="caption" fontWeight="bold"
                                                    display="block">{msg.reply_to_sender}</Typography>
                                        <Typography variant="caption"
                                                    noWrap>{msg.reply_to_text}</Typography>
                                    </Box>)}

                                    {!isMe && selectedRoom.type === 'group' &&
                                        <Typography variant="caption" color="secondary.main"
                                                    fontWeight="bold" display="block"
                                                    mb={0.5}>{msg.sender_name}</Typography>}

                                    {/* نمایش فایل */}
                                    {msg.file && (<Box sx={{mb: 1}}>
                                        {isAudio(msg.file) ? (// ✅ پلیر صوتی
                                            <audio controls style={{
                                                maxWidth: '100%', borderRadius: '8px', height: '40px'
                                            }}>
                                                <source src={msg.file}/>
                                                مرورگر شما پشتیبانی نمی‌کند.
                                            </audio>) : isImage(msg.file) ? (
                                            <img src={msg.file} alt="attachment" style={{
                                                maxWidth: '100%',
                                                borderRadius: 8,
                                                cursor: 'pointer',
                                                maxHeight: 300,
                                                objectFit: 'cover'
                                                 ...
                                                 onClick={() => window.open(msg.file, '_blank')}/>) : isVideo(msg.file) ? (
                                            <video src={msg.file} controls
                                                   style={{maxWidth: '100%', borderRadius: 8}}/>) : (
                                            <Button variant="outlined" size="small" href={msg.file}
                                                    target="_blank" startIcon={<FileIcon/>} sx={{
                                                color: isMe ? '#fff' : theme.palette.text.primary,
                                                borderColor: isMe ? 'rgba(255,255,255,0.5)' : theme.palette.divider
                                            }}>دانلود فایل</Button>)}
                                    </Box>)}
                                    ...
                                    <Stack direction="row" alignItems="center" spacing={0.5}
                                           justifyContent="flex-end" mt={0.5} sx={{opacity: 0.7}}>
                                        <Typography variant="caption"
                                                    sx={{fontSize: '0.65rem'}}>{moment(msg.created_at).locale('fa').format('HH:mm')}</Typography>
                                        {isMe && (msg.is_read ? <DoneAllIcon sx={{fontSize: 14, color: '#4fc3f7'}}/> :
                    ...
                    {replyTo && (<Box sx={{
                        ...
                        <Box sx={{borderRight: '3px solid', borderColor: 'primary.main', pr: 1}}>
                            <Typography variant="caption" color="primary.main" fontWeight="bold">پاسخ
                                به {replyTo.sender_name}</Typography>
                            <Typography variant="body2" color="text.secondary"
                                        noWrap>{replyTo.text || 'فایل ضمیمه'}</Typography>
                        </Box>
                        <IconButton size="small" onClick={() => setReplyTo(null)}
                                    sx={{color: theme.palette.text.secondary}}><CloseIcon/></IconButton>
                    </Box>)}

                    {/* ورودی متن */}
                    <Box p={2} bgcolor={alpha(theme.palette.background.paper, 0.8)}
                         borderTop={`1px solid ${theme.palette.divider}`}>
                        {selectedFile && (<Box mb={1}>
                            <Chip
                        ...
                        <Stack direction="row" spacing={1} alignItems="flex-end">
                            <IconButton onClick={(e) => setEmojiAnchorEl(e.currentTarget)}><EmojiIcon
                                color="warning"/></IconButton>
                            <IconButton component="label" sx={{color: theme.palette.text.secondary}}><input
                                hidden type="file"
                                onChange={(e) => setSelectedFile(e.target.files[0])}/><AttachIcon/></IconButton>
                            ...
                            <IconButton color="primary" onClick={handleSendMessage}
                                        disabled={sending || (!messageText.trim() && !selectedFile)} sx={{
                                ...
                                {sending ? <CircularProgress size={20} color="inherit"/> :
                                    <SendIcon sx={{transform: 'rotate(180deg)', ml: 0.5}}/>}
                            </IconButton>
                        </Stack>
                    </Box>
