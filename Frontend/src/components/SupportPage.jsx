...
import {Add as AddIcon, Send as SendIcon, SupportAgent as SupportIcon} from '@mui/icons-material';
import {useSnackbar} from 'notistack';
import moment from 'jalali-moment';
import {createTicket, getTicketMessages, getTickets, sendTicketMessage, updateTicket} from '../api';
        ...
        <Box sx={{
            ...
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                ...
                <Button variant="contained" size="large" startIcon={<AddIcon/>} onClick={() => setOpenModal(true)}
                        sx={{bgcolor: theme.palette.success.main, color: '#fff', fontWeight: 'bold'}}>
                    تیکت جدید
                </Button>
            </Stack>
            ...
            <Grid container spacing={3} sx={{flexGrow: 1, overflow: 'hidden'}}>
                <Grid item xs={12} md={4} sx={{height: '100%'}}>
                    <Paper sx={glassSx}>
                        <Box p={2} bgcolor={alpha(theme.palette.success.main, 0.1)} color={theme.palette.success.main}
                             borderBottom={`1px solid ${theme.palette.divider}`}>
                            <Typography fontWeight="bold">لیست درخواست‌ها</Typography>
                        </Box>
                        <List sx={{flexGrow: 1, overflowY: 'auto', p: 0}}>
                            {tickets.map((ticket) => (
                                <ListItem key={ticket.id} button onClick={() => setSelectedTicket(ticket)}
                                          selected={selectedTicket?.id === ticket.id} divider sx={{
                                    '&.Mui-selected': {
                                    ...
                                    <ListItemText
                                        primary={<Typography fontWeight="bold" color="text.primary"
                                                             noWrap>{ticket.title}</Typography>}
                                        secondary={<Stack direction="row" justifyContent="space-between"
                                                          mt={0.5}><Typography variant="caption"
                                                                               color="text.secondary">{STATUS_LABELS[ticket.status]}</Typography><Typography
                                            variant="caption"
                                            color="text.secondary">{moment(ticket.updated_at).locale('fa').fromNow()}</Typography></Stack>}
                                    />
                                </ListItem>
                            ))}
                        </List>
                ...
                <Grid item xs={12} md={8} sx={{height: '100%'}}>
                    <Paper sx={glassSx}>
                        {selectedTicket ? (
                            <>
                                <Box p={2} borderBottom={`1px solid ${theme.palette.divider}`} display="flex"
                                     justifyContent="space-between" alignItems="center"
                                     bgcolor={alpha(theme.palette.background.default, 0.5)}>
                                    <Box><Typography variant="h6">{selectedTicket.title}</Typography><Typography
                                        variant="caption"
                                        color="text.secondary">#{selectedTicket.id} | {STATUS_LABELS[selectedTicket.status]}</Typography></Box>
                                    {selectedTicket.status !== 'closed' &&
                                        <Button variant="outlined" color="error" size="small"
                                                onClick={handleCloseTicket}>بستن</Button>}
                                </Box>
                                            ...
                                            <Box key={msg.id} sx={{
                                                ...
                                                <Paper sx={{
                                                    ...
                                                    <Typography variant="body1"
                                                                sx={{whiteSpace: 'pre-wrap'}}>{msg.text}</Typography>
                                                </Paper>
                                                <Typography variant="caption" sx={{
                                                    mt: 0.5,
                                                    opacity: 0.6,
                                                    color: 'text.secondary'
                                                }}>{moment(msg.created_at).locale('fa').format('HH:mm')}</Typography>
                                            </Box>
                                        );
                                    })}
                                ...
                                <Box p={2} borderTop={`1px solid ${theme.palette.divider}`}
                                     bgcolor={alpha(theme.palette.background.default, 0.5)}>
                                    {selectedTicket.status === 'closed' ? <Alert severity="info" sx={{
                                        bgcolor: alpha(theme.palette.info.main, 0.1),
                                        color: theme.palette.info.main
                                    }}>این تیکت بسته شده است.</Alert> : (
                                        <Stack direction="row" spacing={2}>
                                            <TextField fullWidth placeholder="پیام..." value={messageText}
                                                       onChange={(e) => setMessageText(e.target.value)} multiline
                                                       maxRows={3} sx={textFieldSx}/>
                                            <Button variant="contained" onClick={handleSendMessage}
                                                    disabled={!messageText.trim()}
                                                    sx={{bgcolor: 'primary.main'}}><SendIcon
                                                sx={{transform: 'rotate(180deg)'}}/></Button>
                                        </Stack>
                                    )}
                                </Box>
                            </>
                            ...
                            <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center"
                                 height="100%" opacity={0.5}><SupportIcon
                                sx={{fontSize: 80, mb: 2, color: theme.palette.text.secondary}}/><Typography
                                color="text.secondary">یک تیکت را انتخاب کنید.</Typography></Box>
                        )}
                    </Paper>
                </Grid>
            </Grid>
            ...
            >
                ...
                <DialogContent>
                    <Stack spacing={3} mt={2}>
                        <TextField label="موضوع" fullWidth value={newTicketData.title}
                                   onChange={(e) => setNewTicketData({...newTicketData, title: e.target.value})}
                                   sx={textFieldSx}/>
                        <FormControl fullWidth sx={textFieldSx}>
                            <InputLabel>اولویت</InputLabel>
                            <Select value={newTicketData.priority} label="اولویت"
                                    onChange={(e) => setNewTicketData({...newTicketData, priority: e.target.value})}>
                                <MenuItem value="low">عادی</MenuItem><MenuItem value="high">فوری</MenuItem><MenuItem
                                value="critical">بحرانی</MenuItem>
                            </Select>
                        </FormControl>
                        <TextField label="توضیحات" fullWidth multiline rows={4} value={newTicketData.description}
                ...
                <DialogActions sx={{p: 2, borderTop: `1px solid ${theme.palette.divider}`}}>
                    ...
                    <Button variant="contained" onClick={handleCreateTicket} disabled={creating || !newTicketData.title}
                            sx={{bgcolor: theme.palette.success.main, color: '#fff'}}>ایجاد</Button>
                </DialogActions>
            </Dialog>
        </Box>
    );
