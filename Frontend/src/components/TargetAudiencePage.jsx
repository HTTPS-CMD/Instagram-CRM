...
function TargetAudiencePage() {
    ...
    return (
        <Box sx={{width: '100%', maxWidth: '1600px', mx: 'auto'}}>
                ...
                {audiences.length === 0 ? (
                    ...
                    audiences.map((item) => (
                        <Grid item xs={12} md={6} lg={4} key={item.id}>
                            <ExpandableCard item={item} onEdit={() => openEdit(item)}
            ...
            >
                ...
                <DialogContent dividers sx={{borderColor: theme.palette.divider}}>
                    <Grid container spacing={3} mt={0}>
                        <Grid item xs={12} md={6}><TextField sx={textFieldSx} fullWidth
                                                             label="عنوان (مثلاً: پزشکان زیبایی)" value={formData.title}
                                                             onChange={e => setFormData({
                                                                 ...formData,
                                                                 title: e.target.value
                                                             })}/></Grid>
                        <Grid item xs={12} md={6}>
                            <TextField sx={textFieldSx} select fullWidth label="آیکون مرتبط" value={formData.icon_name}
                        ...
                        <Grid item xs={6}><TextField sx={textFieldSx} fullWidth label="بازه سنی"
                                                     value={formData.age_range} onChange={e => setFormData({
                            ...formData,
                            age_range: e.target.value
                        })}/></Grid>
                        <Grid item xs={6}><TextField sx={textFieldSx} fullWidth label="جنسیت" value={formData.gender}
                                                     onChange={e => setFormData({
                                                         ...formData,
                                                         gender: e.target.value
                                                     })}/></Grid>
                        <Grid item xs={6}><TextField sx={textFieldSx} fullWidth label="شغل/سمت"
                                                     value={formData.job_title} onChange={e => setFormData({
                            ...formData,
                            job_title: e.target.value
                        })}/></Grid>
                        <Grid item xs={6}><TextField sx={textFieldSx} fullWidth label="درآمد"
                                                     value={formData.income_level} onChange={e => setFormData({
                            ...formData,
                            income_level: e.target.value
                        })}/></Grid>
                        <Grid item xs={12}><TextField sx={textFieldSx} fullWidth multiline rows={3}
                                                      label="دغدغه‌ها (Pain Points)" value={formData.pain_points}
                                                      onChange={e => setFormData({
                                                          ...formData,
                                                          pain_points: e.target.value
                                                      })}/></Grid>
                        <Grid item xs={12}><TextField sx={textFieldSx} fullWidth multiline rows={3} label="اهداف"
                                                      value={formData.goals} onChange={e => setFormData({
                            ...formData,
                            goals: e.target.value
                        })}/></Grid>
                        <Grid item xs={12}><TextField sx={textFieldSx} fullWidth multiline rows={3} label="راه‌حل ما"
                                                      value={formData.our_solution} onChange={e => setFormData({
                            ...formData,
                            our_solution: e.target.value
                        })}/></Grid>
                    </Grid>
                </DialogContent>
                <DialogActions sx={{p: 3, borderTop: `1px solid ${theme.palette.divider}`}}>
                    ...
                    <Button onClick={handleSubmit} variant="contained"
                            sx={{bgcolor: '#f06292', color: '#fff', fontWeight: 'bold'}}>ذخیره</Button>
                </DialogActions>
            </Dialog>
            ...
            >
                <DialogTitle sx={{color: theme.palette.success.main, display: 'flex', gap: 1}}><AIIcon/> تولید هوشمند
                    پرسونا</DialogTitle>
                <DialogContent>
                    <Typography variant="body2" sx={{color: theme.palette.text.secondary, mb: 2}}>
                        مخاطب هدف خود را در یک کلمه بنویسید (مثلاً: "کافه‌دارها" یا "فروشندگان لباس"). هوش مصنوعی تمام
                        جزئیات را برای شما تحلیل می‌کند.
                    </Typography>
                    <TextField
                        sx={textFieldSx}
                        fullWidth
                ...
                <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Avatar sx={{
                            ...
                            color: themeColor,
                            border: `2px solid ${alpha(themeColor, 0.5)}`,
                            boxShadow: `0 0 15px ${alpha(themeColor, 0.4)}`
                        }}>
                    ...
                    <Stack direction="row">
                        ...
                        <IconButton size="small" onClick={onDelete} color="error"><DeleteIcon
                    ...
                    <Stack spacing={2}>
                        <Box sx={{
                            ...
                            <Typography variant="body2"
                                        sx={{color: theme.palette.text.primary, mt: 1, lineHeight: 1.6}}>
                                {item.pain_points || '---'}
                            </Typography>
                        </Box>
