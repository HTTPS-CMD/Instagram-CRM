...
function TabPanel({children, value, index, ...other}) {
    return (
        <div role="tabpanel" hidden={value !== index} id={`tabpanel-${index}`} {...other} style={{width: '100%'}}>
            {value === index && (
                <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{duration: 0.4}}><Box
                    sx={{py: 3}}>{children}</Box></motion.div>)}
        </div>
    );
}
...
function ProjectDetailPage() {
    ...
    const handleSaveWeekly = async (weekNumber, newContent) => {
        ...
        try {
            ...
            if (existingReport) setWeeklyReports(prev => prev.map(r => r.id === response.data.id ? response.data : r)); else setWeeklyReports(prev => [...prev, response.data]);
            enqueueSnackbar('ذخیره شد', {variant: 'success'});
        } catch (err) {
            enqueueSnackbar('خطا', {variant: 'error'});
        } finally {
    ...
    const formatEventForCalendar = () => events.map(event => ({
        ...
        color: event.type === 'filming' ? theme.palette.warning.main : (event.type === 'meeting' ? theme.palette.secondary.main : theme.palette.primary.main)
    }));
    ...
    return (
        <motion.div initial={{opacity: 0}} animate={{opacity: 1}} transition={{duration: 0.6}}>
            ...
            <Paper elevation={0} sx={{
                ...
                <IconButton sx={{position: 'absolute', top: 20, left: 20, color: theme.palette.text.secondary}}
                            onClick={() => navigate('/dashboard')}><ArrowBackIcon/></IconButton>
                <Stack direction={{xs: 'column', md: 'row'}} justifyContent="space-between" alignItems="center"
                       spacing={3}>
                    <Stack direction="row" spacing={3} alignItems="center">
                        <Avatar src={project.page_logo} sx={{
                            width: 100,
                            height: 100,
                            ...
                            {project.project_type === 'teaser' ?
                                <MovieIcon fontSize="inherit"/> : project.project_name.charAt(0)}
                        </Avatar>
                        <Box>
                            ...
                            <Stack direction="row" spacing={1} alignItems="center">
                                {project.page_username && <Chip icon={<InstagramIcon sx={{fontSize: 18}}/>}
                                                                label={`@${project.page_username}`} sx={{
                                ...
                                <Chip
                                    ...
                                    color={project.project_type === 'teaser' ? "warning" : "primary"} variant="outlined"
                                    size="small"/>
                            </Stack>
                        </Box>
                    </Stack>
                    {isAdmin && (<Stack direction="row" spacing={2}><Button variant="contained" color="warning"
                                                                            startIcon={<SettingsIcon/>}
                                                                            onClick={handleOpenEditModal} sx={{
                        borderRadius: 3,
                        px: 3
                    }}>تنظیمات</Button><Button variant="outlined" color="error" startIcon={<DeleteIcon/>}
                                               onClick={handleDeleteProject} sx={{borderRadius: 3, px: 3}}>حذف</Button></Stack>)}
                </Stack>
            </Paper>
            ...
            <Paper elevation={0} sx={{bgcolor: 'transparent', mb: 3}}>
                ...
                >
                    {visibleTabs.map((tab) => (<Tab key={tab.id} label={tab.label} icon={tab.icon} iconPosition="start"
                        ...
                        <Grid item xs={12} md={project.project_type === 'teaser' ? 12 : 8}>
                            <Paper sx={{...glassCardStyle, p: 4, height: '100%'}}>
                                <Typography variant="h6" gutterBottom color="primary" sx={{
                                    ...
                                    gap: 1,
                                    fontWeight: 'bold'
                                }}><BadgeIcon/> هویت بصری</Typography>
                                <Divider sx={{mb: 3, borderColor: theme.palette.divider}}/>
                                <Grid container spacing={4}>
                                    <Grid item xs={12} sm={6}><Typography variant="subtitle2" color="text.secondary"
                                                                          gutterBottom>نام پروژه</Typography><Typography
                                        variant="h6" fontWeight="bold"
                                        color="text.primary">{project.project_name}</Typography></Grid>
                                    <Grid item xs={12} sm={6}><Typography variant="subtitle2" color="text.secondary"
                                                                          gutterBottom>نوع
                                        قرارداد</Typography><Typography variant="h6" fontWeight="bold"
                                                                        color="text.primary">{project.project_type === 'teaser' ? 'پروژه تکی / تیزر' : 'مدیریت ماهانه'}</Typography></Grid>
                                    {project.page_username &&
                                        <Grid item xs={12} sm={6}><Typography variant="subtitle2" color="text.secondary"
                                                                              gutterBottom>آیدی
                                            صفحه</Typography><Typography variant="h6" dir="ltr" sx={{
                                            textAlign: 'right',
                                            fontFamily: 'monospace',
                                            color: 'text.primary'
                                        }}>@{project.page_username}</Typography></Grid>}
                                </Grid>
                            </Paper>
                        </Grid>
                        {project.project_type !== 'teaser' && (
                            <Grid item xs={12} md={4}>
                                <Stack spacing={3}>
                                    <Paper sx={{
                                        ...
                                        <Typography variant="subtitle1" sx={{opacity: 0.9, mb: 1}}>تعهد
                                            ماهانه</Typography>
                                        <Typography variant="h2"
                                                    fontWeight="900">{project.monthly_post_goal}</Typography>
                                    ...
                                    <Paper sx={{...glassCardStyle, p: 3}}>
                                        <Typography variant="h6" gutterBottom sx={{
                                            ...
                                            gap: 1,
                                            color: 'info.main'
                                        }}><DateRangeIcon/> زمان‌بندی</Typography>
                                        <Divider sx={{mb: 2}}/>
                                        <Stack spacing={2}>
                                            <Box sx={{
                                                display: 'flex',
                                                ...
                                                <Typography variant="body1" fontWeight="bold"
                                                            color="text.primary">{project.start_date ? moment(project.start_date).locale('fa').format('jD jMMMM jYYYY') : '---'}</Typography>
                                            </Box>
                                            <Box sx={{
                                                display: 'flex',
                                                justifyContent: 'space-between',
                                                ...
                                                <Typography variant="body1" fontWeight="bold"
                                                            color="text.primary">{project.end_date ? moment(project.end_date).locale('fa').format('jD jMMMM jYYYY') : '---'}</Typography>
                                            </Box>
                                        </Stack>
                                    </Paper>
                                </Stack>
                    ...
                    <Grid container spacing={3}>
                        <Grid item xs={12} md={8}>
                            <Paper sx={{...glassCardStyle, p: 2, direction: 'ltr'}}>
                                <FullCalendar plugins={[dayGridPlugin, momentPlugin]} initialView="dayGridMonth"
                                              events={formatEventForCalendar()} locale="fa" direction="rtl" firstDay={6}
                                              headerToolbar={{start: 'title', center: '', end: 'today prev,next'}}
                                              buttonText={{today: 'امروز'}} height="auto" contentHeight="auto"
                                              dayHeaderClassNames="calender-header-rtl"/>
                            </Paper>
                        </Grid>
                        <Grid item xs={12} md={4}>
                            <Paper sx={{...glassCardStyle, p: 0, overflow: 'hidden'}}>
                                <Box p={2} borderBottom={`1px solid ${theme.palette.divider}`}><Typography variant="h6"
                                                                                                           fontWeight="bold"
                                                                                                           color="text.primary">رویدادها</Typography></Box>
                                <List sx={{maxHeight: 400, overflow: 'auto'}}>
                                    {events.length === 0 ? <Typography p={3} align="center"
                                                                       color="text.secondary">خالی</Typography> : events.map((ev) => (
                                        <ListItem key={ev.id} divider>
                                            <ListItemText primary={ev.title} secondary={ev.dateJalali}
                                                          sx={{textAlign: 'right', color: 'text.primary'}}
                                                          secondaryTypographyProps={{color: 'text.secondary'}}/>
                                            <Chip
                                                label={ev.type === 'filming' ? 'آفیش' : (ev.type === 'meeting' ? 'جلسه' : 'پست')}
                                                size="small"
                                                color={ev.type === 'filming' ? 'warning' : (ev.type === 'meeting' ? 'secondary' : 'info')}
                                                variant="outlined"/>
                                            {isAdmin &&
                                                <IconButton onClick={() => handleDeleteEvent(ev.id)} size="small"
                                                            color="error"><DeleteIcon/></IconButton>}
                                        </ListItem>
                                    ))}
                                </List>
                                {isAdmin && <Box p={2}><Button fullWidth variant="contained" startIcon={<AddIcon/>}
                                                               onClick={() => setOpenEventModal(true)}>افزودن</Button></Box>}
                            </Paper>
                        </Grid>
                    </Grid>
                </TabPanel>
                ...
                <TabPanel value={currentTab} index={currentTab}>
                    <Paper sx={{...glassCardStyle, p: 3, minHeight: 500}}>
                        <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                            ...
                            <ToggleButtonGroup value={reportViewMode} exclusive
                                               onChange={(e, v) => v && setReportViewMode(v)} size="small" color="info">
                                <ToggleButton value="weekly" sx={{px: 3}}><WeeklyIcon
                                    sx={{mr: 1}}/> هفتگی</ToggleButton>
                                <ToggleButton value="monthly" sx={{px: 3}}><MonthlyIcon
                                    sx={{mr: 1}}/> ماهانه</ToggleButton>
                            </ToggleButtonGroup>
                        </Stack>
                        <Divider sx={{mb: 3}}/>
                            ...
                            <Stack spacing={3}>
                                {[1, 2, 3, 4, 5].map(week => (
                                    <Paper key={week} sx={{
                                        ...
                                        <ReportEditor title={`گزارش هفته ${week}`}
                                                      initialContent={getWeeklyContent(week)}
                                                      onSave={(c) => handleSaveWeekly(week, c)}
                                                      isSaving={weeklySavingStatus[week]} isAdmin={isAdmin}/>
                                    </Paper>
                                ))}
                            </Stack>
                        ) : (
                            <Box sx={{mt: 2}}><ReportEditor title="گزارش نهایی ماهانه"
                                                            initialContent={project.monthly_report_text}
                                                            onSave={handleSaveMonthly} isSaving={isSavingMonthly}
                                                            isAdmin={isAdmin}/></Box>
                        )}
                    </Paper>
                </TabPanel>
            ...
            {visibleTabs[currentTab]?.id === 'analysis' && (
                <TabPanel value={currentTab} index={currentTab}><AIAnalysisTab projectId={projectId}/></TabPanel>)}
            {visibleTabs[currentTab]?.id === 'media' && (
                <TabPanel value={currentTab} index={currentTab}><Paper sx={glassCardStyle}
                                                                       elevation={0}><MediaManagement
                    project={project}/></Paper></TabPanel>)}
            {visibleTabs[currentTab]?.id === 'financials' && (
                <TabPanel value={currentTab} index={currentTab}><FinancialManagement project={project} isAdmin={isAdmin}
                                                                                     onProjectUpdate={handleProjectUpdate}/></TabPanel>)}
            ...
            <Dialog open={openEditModal} onClose={() => setOpenEditModal(false)} maxWidth="md" fullWidth>
                ...
                <DialogContent dividers>
                    <Grid container spacing={3} sx={{mt: 0}}>
                        <Grid item xs={12} md={6}><TextField label="نام پروژه" name="project_name"
                                                             value={editFormData.project_name || ''}
                                                             onChange={handleEditChange} fullWidth required/></Grid>
                        <Grid item xs={12} md={6}>
                            <FormControl fullWidth>
                                <InputLabel>تغییر مشتری (مالک)</InputLabel>
                                <Select name="client_user" value={editFormData.client_user} label="تغییر مشتری (مالک)"
                                        onChange={handleEditChange} sx={selectStyle}>
                        ...
                        {project && project.project_type === 'instagram' && (
                            <>
                                <Grid item xs={12} md={6}><TextField label="آیدی صفحه" name="page_username"
                                                                     value={editFormData.page_username || ''}
                                                                     onChange={handleEditChange} fullWidth/></Grid>
                                <Grid item xs={12} md={4}><TextField label="تعهد ماهانه" name="monthly_post_goal"
                                                                     type="number"
                                                                     value={editFormData.monthly_post_goal || ''}
                                                                     onChange={handleEditChange} fullWidth/></Grid>
                            </>
                        )}
                        <Grid item xs={12} md={4}><DatePicker label="شروع" value={editFormData.start_date}
                                                              onChange={(v) => handleDateChange('start_date', v)}
                                                              renderInput={(p) => <TextField {...p}
                                                                                             fullWidth/>}/></Grid>
                        <Grid item xs={12} md={4}><DatePicker label="پایان" value={editFormData.end_date}
                    ...
                    <Typography variant="subtitle1" color="primary"
                                sx={{mb: 2, display: 'flex', alignItems: 'center', gap: 1}}><GroupIcon/> ویرایش تیم
                        اجرایی</Typography>
                    <Grid container spacing={3}>
                        <Grid item xs={12} md={6}><FormControl fullWidth><InputLabel>تیم نویسندگان</InputLabel><Select
                            name="writers" multiple value={editFormData.writers || []} label="تیم نویسندگان"
                            onChange={handleEditChange} sx={selectStyle}
                            renderValue={(selected) => selected.map(id => writers.find(u => u.id === id)?.username).join(', ')}>{writers.map(u =>
                            <MenuItem key={u.id} value={u.id}>{u.username}</MenuItem>)}</Select></FormControl></Grid>
                        <Grid item xs={12} md={6}><FormControl fullWidth><InputLabel>تیم فیلم‌برداری</InputLabel><Select
                            name="videographers" multiple value={editFormData.videographers || []}
                            label="تیم فیلم‌برداری" onChange={handleEditChange} sx={selectStyle}
                            renderValue={(selected) => selected.map(id => videographers.find(u => u.id === id)?.username).join(', ')}>{videographers.map(u =>
                            <MenuItem key={u.id} value={u.id}>{u.username}</MenuItem>)}</Select></FormControl></Grid>
                        <Grid item xs={12} md={6}><FormControl fullWidth><InputLabel>تیم تدوین</InputLabel><Select
                            name="editors" multiple value={editFormData.editors || []} label="تیم تدوین"
                            onChange={handleEditChange} sx={selectStyle}
                            renderValue={(selected) => selected.map(id => editors.find(u => u.id === id)?.username).join(', ')}>{editors.map(u =>
                            <MenuItem key={u.id} value={u.id}>{u.username}</MenuItem>)}</Select></FormControl></Grid>
                        <Grid item xs={12} md={6}><FormControl fullWidth><InputLabel>تیم گرافیک</InputLabel><Select
                            name="designers" multiple value={editFormData.designers || []} label="تیم گرافیک"
                            onChange={handleEditChange} sx={selectStyle}
                            renderValue={(selected) => selected.map(id => designers.find(u => u.id === id)?.username).join(', ')}>{designers.map(u =>
                            <MenuItem key={u.id} value={u.id}>{u.username}</MenuItem>)}</Select></FormControl></Grid>
                        <Grid item xs={12} md={6}><FormControl fullWidth><InputLabel>تیم ادمین سوشال</InputLabel><Select
                            name="social_admins" multiple value={editFormData.social_admins || []}
                            label="تیم ادمین سوشال" onChange={handleEditChange} sx={selectStyle}
                            renderValue={(selected) => selected.map(id => socialAdmins.find(u => u.id === id)?.username).join(', ')}>{socialAdmins.map(u =>
                            <MenuItem key={u.id} value={u.id}>{u.username}</MenuItem>)}</Select></FormControl></Grid>
                    </Grid>
                </DialogContent>
                <DialogActions><Button onClick={() => setOpenEditModal(false)}>انصراف</Button><Button
                    onClick={handleEditSubmit} variant="contained" color="warning">ذخیره</Button></DialogActions>
            </Dialog>
        </motion.div>
    );
}
