...
import {
    ...
    MenuItem, Select, InputLabel, FormControl, Avatar, Tooltip, CircularProgress, alpha, useTheme
} from '@mui/material';
import { Add as AddIcon, Edit as EditIcon, Delete as DeleteIcon, Flag as FlagIcon, AccessTime as TimeIcon } from '@mui/icons-material';
...
function TaskKanban({ projectId }) {
    ...
    const onDragEnd = async (result) => {
        ...
        if (source.droppableId !== destination.droppableId) {
            ...
            try { await updateTask(projectId, draggableId, { status: destination.droppableId }); }
            catch (err) { enqueueSnackbar('خطا در تغییر وضعیت', { variant: 'error' }); fetchData(); }
        }
    };
    ...
    const handleOpenModal = (task = null) => {
        if (task) {
            ...
            setEditingTask(null);
            setFormData({ title: '', description: '', priority: 'medium', assigned_to: '', due_date: null, status: 'todo' });
        }
        setOpenModal(true);
    };

    const handleSubmit = async () => {
        if (!formData.title) { enqueueSnackbar('عنوان تسک الزامی است', { variant: 'warning' }); return; }
        try {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('آیا مطمئن هستید؟')) {
            try { await deleteTask(projectId, id); setTasks(prev => prev.filter(t => t.id !== id)); }
    ...
    return (
        <Box>
            <Stack direction="row" justifyContent="flex-end" mb={3}>
                <Button variant="contained" startIcon={<AddIcon />} onClick={() => handleOpenModal()} sx={{ bgcolor: 'primary.main', fontWeight: 'bold' }}>تسک جدید</Button>
            </Stack>
                        ...
                        <Paper key={colId} sx={columnStyle(col.color)}>
                            <Box p={2} borderBottom={`3px solid ${col.color}`} bgcolor={alpha(col.color, 0.1)}>
                                ...
                                <Box display="flex" alignItems="center">
                                    ...
                                    <Chip label={tasks.filter(t => t.status === colId).length} size="small" sx={{ ml: 1, bgcolor: col.color, color: '#fff', fontWeight: 'bold' }} />
                                </Box>
                            </Box>
                            <Droppable droppableId={colId}>
                                {(provided) => (
                                    <Box ref={provided.innerRef} {...provided.droppableProps} sx={{ flexGrow: 1, overflowY: 'auto', p: 2 }}>
                                        {tasks.filter(t => t.status === colId).map((task, index) => (
                                            <Draggable key={task.id} draggableId={task.id.toString()} index={index}>
                                                {(provided) => (
                                                    <Box ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} sx={{ ...provided.draggableProps.style, mb: 2 }}>
                                                        <Paper sx={{ p: 2, borderRadius: 3, bgcolor: alpha(theme.palette.background.default, 0.5), border: `1px solid ${theme.palette.divider}`, boxShadow: theme.shadows[2] }}>
                                                            <Stack direction="row" justifyContent="space-between" alignItems="start">
                                                                <Typography variant="subtitle2" fontWeight="bold">{task.title}</Typography>
                                                                <Stack direction="row">
                                                                    <IconButton size="small" onClick={() => handleOpenModal(task)} color="info"><EditIcon fontSize="small" /></IconButton>
                                                                    <IconButton size="small" onClick={() => handleDelete(task.id)} color="error"><DeleteIcon fontSize="small" /></IconButton>
                                                                </Stack>
                                                            </Stack>
                                                            <Typography variant="caption" color="text.secondary" display="block" mb={2} sx={{whiteSpace:'pre-wrap'}}>{task.description}</Typography>
                                                            <Stack direction="row" justifyContent="space-between" alignItems="center">
                                                                <Stack direction="row" spacing={1} alignItems="center">
                                                                    <TimeTracker taskId={task.id} />
                                                                    {(task.assigned_to_details) && (
                                                                        <Tooltip title={task.assigned_to_details.full_name || task.assigned_to_details.username}>
                                                                            <Avatar src={task.assigned_to_details.avatar} sx={{ width: 24, height: 24 }} />
                                                                        </Tooltip>
                                                                    )}
                                                                    {task.due_date && <Chip icon={<TimeIcon sx={{fontSize:14}}/>} label={moment(task.due_date).format('jMM/jDD')} size="small" sx={{ height: 24, fontSize: '0.7rem' }} />}
                                                                </Stack>
                                                                <Chip icon={<FlagIcon sx={{fontSize:14}}/>} label={task.priority} size="small" color={PRIORITY_COLORS[task.priority]} variant="outlined" sx={{ height: 20, fontSize: '0.6rem' }} />
                                                            </Stack>
                                                        </Paper>
                                                    </Box>
                                                )}
            ...
            <Dialog open={openModal} onClose={() => setOpenModal(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: theme.palette.background.paper } }}>
                <DialogTitle sx={{borderBottom: `1px solid ${theme.palette.divider}`}}>{editingTask ? 'ویرایش تسک' : 'تسک جدید'}</DialogTitle>
                <DialogContent>
                    <Stack spacing={2} mt={2}>
                        <TextField label="عنوان" fullWidth value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} sx={textFieldSx(theme)} />
                        <TextField label="توضیحات" fullWidth multiline rows={3} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} sx={textFieldSx(theme)} />
                        <Stack direction="row" spacing={2}>
                            <FormControl fullWidth sx={textFieldSx(theme)}>
                                <InputLabel>اولویت</InputLabel>
                                <Select value={formData.priority} label="اولویت" onChange={e => setFormData({...formData, priority: e.target.value})}>
                            ...
                            <FormControl fullWidth sx={textFieldSx(theme)}>
                                ...
                                <Select value={formData.assigned_to} label="مسئول انجام" onChange={e => setFormData({...formData, assigned_to: e.target.value})}>
                        ...
                        <DatePicker label="مهلت انجام" value={formData.due_date} onChange={v => setFormData({...formData, due_date: v})} renderInput={(p) => <TextField {...p} sx={textFieldSx(theme)} fullWidth />} />
                    </Stack>
                </DialogContent>
