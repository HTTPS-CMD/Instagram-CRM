...
function FinancialManagement({project, onProjectUpdate, isAdmin}) {
    ...
    const handlePrintInvoice = useReactToPrint({
        contentRef: invoiceRef, documentTitle: `Invoice-${printPayment?.id}`, onAfterPrint: () => setPrintPayment(null),
    });
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            ...
            try {
                const basePromises = [getPackages(), getPaymentMethods(), getProjectPayments(project.id), getAgencyInfo()];
    ...
    const handleCreatePayment = async () => {
        ...
        try {
            const data = {
                ...
                date: paymentData.date.locale('en').format('YYYY-MM-DD'),
                is_paid: true
            };
    ...
    const handleExportExcel = async () => {
        try {
            ...
            enqueueSnackbar('خطا در دانلود', {variant: 'error'});
        }
    };
    ...
    const textFieldSx = {
        '& .MuiInputLabel-root': {color: theme.palette.text.secondary}, '& .MuiOutlinedInput-root': {
            color: theme.palette.text.primary, '& fieldset': {borderColor: theme.palette.divider}
        }
    };
    const tableHeadSx = {
        '& th': {
    ...
    const tableBodySx = {
        ...
        }, '& tr:hover': {bgcolor: `${alpha(theme.palette.action.hover, 0.1)} !important`}
    };
    ...
    return (<Box>
        <Paper elevation={3} sx={{
            ...
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                <Typography variant="h6"
                    ...
                    وضعیت: <span
                    style={{color: project.is_started ? theme.palette.success.main : theme.palette.error.main}}>{project.is_started ? 'فعال' : 'غیرفعال'}</span>
                </Typography>
                {isAdmin && <FormControlLabel
                    control={<Switch checked={project.is_started || false} onChange={handleToggleStart}/>}
                    label="تغییر وضعیت" sx={{color: theme.palette.text.primary}}/>}
            </Stack>
            ...
            <Grid container spacing={2}>
                <Grid item xs={12} md={8}>
                    ...
                    <Grid container spacing={2}>
                        {packagesList.map((pkg) => (<Grid item xs={12} sm={6} key={pkg.id}>
                            <Card sx={{
                                ...
                                <CardActionArea onClick={() => handleSelectionChange('package', pkg.id)}
                                                disabled={!isAdmin} sx={{p: 2}}>
                                    <Typography fontWeight="bold">{pkg.title}</Typography>
                                    <Typography variant="h6" fontWeight="900"
                                                mt={1}>{formatPrice(pkg.price)} تومان</Typography>
                                </CardActionArea>
                            </Card>
                        </Grid>))}
                    </Grid>
                ...
                <Grid item xs={12} md={4}>
                    ...
                    <Stack spacing={1}>
                        {methodsList.map((method) => (<Card key={method.id} sx={{
                            ...
                            <CardActionArea onClick={() => handleSelectionChange('method', method.id)}
                                            disabled={!isAdmin}
                                            sx={{p: 1.5, display: 'flex', alignItems: 'center', gap: 1}}>
                                {method.id === selectedMethodId ? <CheckIcon color="secondary" fontSize="small"/> :
                                    <UncheckedIcon fontSize="small" color="action"/>}
                                <Typography variant="body2">{method.title}</Typography>
                                {method.discount_percent > 0 &&
                                    <Chip label={`-${method.discount_percent}%`} color="error" size="small"
            ...
            <Grid item xs={12} sm={6} md={3}>
                <Paper sx={{
                    ...
                    <Typography variant="h5" fontWeight="900"
                                color="success.main">{formatPrice(totalReceived)}</Typography>
                </Paper>
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
                <Paper sx={{
                    ...
                    <Typography variant="h5" fontWeight="900"
                                color="info.main">{formatPrice(contractAmount)}</Typography>
                </Paper>
            </Grid>
            {isAdmin && (<>
                <Grid item xs={12} sm={6} md={3}>
                    <Paper sx={{
                        ...
                        <Typography variant="caption" display="block"
                                    color="text.secondary">هزینه‌ها</Typography>
                        <Typography variant="h5" fontWeight="900"
                                    color="error.main">{formatPrice(totalExpenses)}</Typography>
                    </Paper>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <Paper sx={{
                        ...
                        <Typography variant="caption" display="block" color="text.secondary">سود
                            خالص</Typography>
                        <Typography variant="h5" fontWeight="900"
                                    color="warning.main">{formatPrice(netProfit)}</Typography>
                    </Paper>
                </Grid>
            </>)}
        </Grid>
        ...
        <Typography variant="h6" gutterBottom sx={{
            display: 'flex', alignItems: 'center', gap: 1, mt: 4, color: theme.palette.text.primary
        }}><DateIcon color="primary"/> برنامه زمان‌بندی اقساط</Typography>
                    ...
                    <Paper elevation={3} sx={{
                        ...
                        <Stack direction="row" justifyContent="space-between" mb={1}>
                            <Typography variant="subtitle2"
                                        color="text.secondary">قسط {index + 1} ({percent}٪)</Typography>
                            <Chip label={isPaid ? 'پرداخت شده' : 'موعد نرسیده'}
                                  color={isPaid ? "success" : "default"} size="small"
                                  variant={isPaid ? "filled" : "outlined"}/>
                        </Stack>
                        <Typography variant="h6" fontWeight="bold" mb={1}>{formatPrice(amount)} ت</Typography>
                        <Typography variant="caption"
                                    color="text.secondary">سررسید: {dueDate ? dueDate.locale('fa').format('jD jMMMM jYYYY') : '---'}</Typography>
                    </Paper>
                </Grid>);
            }) : (<Grid item xs={12}><Typography color="text.secondary" align="center">روش پرداخت انتخاب نشده
                است.</Typography></Grid>)}
        </Grid>
            ...
            <Grid item xs={12} md={isAdmin ? 6 : 12}>
                <Paper elevation={3} sx={{...glassCardSx, p: 2, borderRadius: 2, height: '100%'}}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                        ...
                        {isAdmin && <Button size="small" variant="contained" startIcon={<AddIcon/>}
                                            onClick={() => setOpenPaymentModal(true)}>ثبت</Button>}
                    </Stack>
                    <TableContainer>
                        <Table size="small">
                            <TableHead
                                sx={tableHeadSx}><TableRow><TableCell>مبلغ</TableCell><TableCell>تاریخ</TableCell><TableCell>شرح</TableCell><TableCell>عملیات</TableCell></TableRow></TableHead>
                            <TableBody sx={tableBodySx}>
                                {payments.map((p) => (<TableRow key={p.id}>
                                    <TableCell sx={{
                                        color: theme.palette.success.main, fontWeight: 'bold'
                                    }}>{formatPrice(p.amount)}</TableCell>
                                    <TableCell>{moment(p.date).locale('fa').format('jYYYY/jMM/jDD')}</TableCell>
                                    <TableCell>{p.description}</TableCell>
                                    <TableCell>
                                        <IconButton size="small" color="info"
                                                    onClick={() => onPrintClick(p)}><PrintIcon
                                            fontSize="small"/></IconButton>
                                        {isAdmin && <IconButton size="small" color="error"
                                                                onClick={() => handleDeletePayment(p.id)}><DeleteIcon
                                            fontSize="small"/></IconButton>}
                                    </TableCell>
                                </TableRow>))}
                            </TableBody>
            ...
            {isAdmin && (<Grid item xs={12} md={6}>
                <Paper elevation={3} sx={{...glassCardSx, p: 2, borderRadius: 2, height: '100%'}}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                        ...
                        <Button size="small" variant="contained" color="error" startIcon={<AddIcon/>}
                                onClick={() => setOpenExpenseModal(true)}>ثبت</Button>
                    </Stack>
                    <TableContainer>
                        <Table size="small">
                            <TableHead
                                sx={tableHeadSx}><TableRow><TableCell>عنوان</TableCell><TableCell>مبلغ</TableCell><TableCell>تاریخ</TableCell><TableCell></TableCell></TableRow></TableHead>
                            <TableBody sx={tableBodySx}>
                                {expenses.map((e) => (<TableRow key={e.id}>
                                    <TableCell>{e.title}</TableCell>
                                    <TableCell fontWeight="bold"
                                               sx={{color: theme.palette.error.main}}>{Number(e.amount).toLocaleString()} ت</TableCell>
                                    <TableCell>{moment(e.date).locale('fa').format('jYYYY/jMM/jDD')}</TableCell>
                                    <TableCell><IconButton size="small" color="error"
                                                           onClick={() => handleDeleteExpense(e.id)}><DeleteIcon
                                        fontSize="small"/></IconButton></TableCell>
                                </TableRow>))}
                            </TableBody>
                        </Table>
                    </TableContainer>
        ...
        >
            ...
            <DialogContent>
                <Stack spacing={2} mt={2}>
                    <TextField sx={textFieldSx} label="مبلغ" type="number" fullWidth value={paymentData.amount}
                               onChange={(e) => setPaymentData({...paymentData, amount: e.target.value})}/>
                    <TextField sx={textFieldSx} label="شرح" fullWidth value={paymentData.description}
                               onChange={(e) => setPaymentData({...paymentData, description: e.target.value})}/>
                    <DatePicker label="تاریخ" value={paymentData.date}
                                onChange={(v) => setPaymentData({...paymentData, date: v})}
                                renderInput={(p) => <TextField {...p} sx={textFieldSx}/>}/>
                </Stack>
            </DialogContent>
            <DialogActions><Button onClick={() => setOpenPaymentModal(false)} color="inherit">لغو</Button><Button
                onClick={handleCreatePayment} variant="contained" color="success">ثبت</Button></DialogActions>
        </Dialog>

        <Dialog open={openExpenseModal} onClose={() => setOpenExpenseModal(false)} maxWidth="xs" fullWidth
                PaperProps={{sx: {bgcolor: theme.palette.background.paper, color: theme.palette.text.primary}}}
        >
            <DialogTitle sx={{color: 'error.main', borderBottom: `1px solid ${theme.palette.divider}`}}>ثبت هزینه
                جدید</DialogTitle>
            <DialogContent>
                <Stack spacing={2} mt={2}>
                    <TextField sx={textFieldSx} label="مبلغ" type="number" fullWidth value={expenseData.amount}
                               onChange={(e) => setExpenseData({...expenseData, amount: e.target.value})}
                               color="error"/>
                    <TextField sx={textFieldSx} label="بابت" fullWidth value={expenseData.description}
                               onChange={(e) => setExpenseData({...expenseData, description: e.target.value})}
                               color="error"/>
                    <DatePicker label="تاریخ" value={expenseData.date}
                                onChange={(v) => setExpenseData({...expenseData, date: v})}
                                renderInput={(p) => <TextField {...p} sx={textFieldSx} color="error"/>}/>
                </Stack>
            </DialogContent>
            <DialogActions><Button onClick={() => setOpenExpenseModal(false)} color="inherit">لغو</Button><Button
                onClick={handleCreateExpense} variant="contained" color="error">ثبت</Button></DialogActions>
        </Dialog>
