...
import {
    ...
    getProjectFiles,
    uploadProjectFile
} from '../api'; // ✅ generateSharedLink ایمپورت شد
...
function MediaManagement({project}) {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            enqueueSnackbar('خطا در آپلود', {variant: 'error'});
        } finally {
            setUploading(false);
        }
    };
    ...
    const handleAddComment = async () => {
        ...
        try {
            ...
            enqueueSnackbar('خطا در ثبت کامنت', {variant: 'error'});
        }
    };
    ...
    return (
        <Box>
            <Stack direction={{xs: 'column', md: 'row'}} justifyContent="space-between" alignItems="center" mb={3}
                ...
                >
                    {FOLDERS.map(f => <Tab key={f.id} value={f.id} label={f.label} icon={f.icon}
                                           iconPosition="start"/>)}
                </Tabs>
                ...
                <Stack direction="row" spacing={1}>
                    {selectedIds.length > 0 ? (
                        <>
                            <Button variant="contained" color="error" startIcon={<DeleteIcon/>} onClick={handleDelete}>حذف
                        ...
                        <>
                            <Button variant="outlined"
                                    sx={{color: theme.palette.text.secondary, borderColor: theme.palette.divider}}
                                    onClick={() => setSelectMode(!selectMode)}>{selectMode ? 'لغو انتخاب' : 'انتخاب گروهی'}</Button>
                            <Button variant="contained" color="secondary" startIcon={<UploadIcon/>}
                                    onClick={() => setOpenUpload(true)} sx={{
            ...
            {loading ? <Box display="flex" justifyContent="center"
                            py={10}><CircularProgress/></Box> : filteredFiles.length === 0 ?
                <Paper sx={{...glassSx, p: 5, textAlign: 'center', borderStyle: 'dashed'}}><Typography
                    color="text.secondary">فایلی یافت نشد.</Typography></Paper> : (
                        ...
                        {filteredFiles.map((file) => {
                            ...
                            return (
                                <Grid item xs={6} sm={4} md={3} lg={2.4} key={file.id}>
                                    <Fade in={true}>
                                        <Box sx={mediaCardSx(isSelected)} onClick={() => {
                                            if (selectMode) {
                                                setSelectedIds(prev => prev.includes(file.id) ? prev.filter(x => x !== file.id) : [...prev, file.id]);
                                            } else {
                                                setPreviewFile(file);
                                            }
                                        }}>
                                            {selectMode &&
                                                <Box sx={{position: 'absolute', top: 8, right: 8, zIndex: 2}}><Checkbox
                                                    checked={isSelected} icon={<UncheckedIcon sx={{color: '#fff'}}/>}
                                                    checkedIcon={<CheckCircleIcon color="primary"/>}/></Box>}
                                            <Box sx={{
                                                height: 160,
                                                bgcolor: '#000',
                                                display: 'flex',
                                                ...
                                                {isImage(file.file) ? <CardMedia component="img" image={file.file} sx={{
                                                    height: '100%',
                                                    objectFit: 'cover'
                                                }}/> : isVideo(file.file) ? <>
                                                        <video src={file.file} style={{
                                                            width: '100%',
                                                            height: '100%',
                                                            objectFit: 'cover',
                                                            opacity: 0.7
                                                    ...
                                                    <FileIcon sx={{fontSize: 60, color: 'rgba(255,255,255,0.5)'}}/>}
                                            </Box>
                                            <Box sx={{
                                                p: 1.5,
                                                ...
                                                <Typography variant="body2" fontWeight="bold" noWrap
                                                            sx={{color: 'white'}}>{file.realDesc || 'بدون عنوان'}</Typography>
                                                <Typography variant="caption" sx={{
                                                    color: 'rgba(255,255,255,0.7)',
                                                    display: 'block'
                                                }}>{file.uploaded_at ? moment(file.uploaded_at).locale('fa').format('jD jMMMM') : ''}</Typography>
                                            </Box>
                                        </Box>
                                    </Fade>
                                </Grid>
            ...
            <Dialog open={!!previewFile} onClose={() => {
                setPreviewFile(null);
                setTempPoint(null);
            ...
            >
                <DialogTitle sx={{
                    ...
                    <IconButton onClick={() => {
                        setPreviewFile(null);
                        setTempPoint(null);
                    }} color="inherit"><CloseIcon/></IconButton>
                </DialogTitle>
                <DialogContent sx={{p: 0, display: 'flex', height: '100%'}}>
                    ...
                    <Box sx={{
                        ...
                        justifyContent: 'center',
                        overflow: 'hidden',
                        cursor: isImage(previewFile?.file) ? 'crosshair' : 'default'
                    }}>
                    ...
                    <Box sx={{
                        ...
                        <Box p={2} borderBottom={`1px solid ${theme.palette.divider}`}>
                            <Typography variant="subtitle1" fontWeight="bold" color="text.primary">نظرات و
                                اصلاحات</Typography>
                            {isImage(previewFile?.file) &&
                                <Typography variant="caption" color="text.secondary">روی تصویر کلیک کنید تا نظر
                                    بدهید.</Typography>}
                        </Box>

                        <Box sx={{flex: 1, overflowY: 'auto', p: 2}}>
                            {loadingComments ? <CircularProgress size={20} sx={{
                                display: 'block',
                                mx: 'auto'
                            }}/> : comments.length === 0 ?
                                <Typography align="center" color="text.secondary" mt={5}>هنوز نظری ثبت نشده
                                    است.</Typography> :
                                comments.map(c => (
                                    <Paper key={c.id}
                                           sx={{p: 1.5, mb: 2, bgcolor: alpha(theme.palette.background.paper, 0.5)}}>
                                        <Stack direction="row" spacing={1} alignItems="center" mb={1}>
                                            <Avatar src={c.author_avatar}
                                                    sx={{width: 24, height: 24}}>{c.author_name?.[0]}</Avatar>
                                            <Typography variant="subtitle2" fontWeight="bold"
                                                        color="primary">{c.author_name}</Typography>
                                            <Typography variant="caption" color="text.secondary"
                                                        sx={{ml: 'auto'}}>{moment(c.created_at).locale('fa').fromNow()}</Typography>
                                            {(user.id === c.author || user.role === 'admin') &&
                                                <IconButton size="small" color="error"
                                                            onClick={() => handleDeleteComment(c.id)}><DeleteIcon
                        ...
                        {isImage(previewFile?.file) && tempPoint ? (
                            <Box p={2} borderTop={`1px solid ${theme.palette.divider}`}
                                ...
                                <Typography variant="caption" color="success.main" mb={1} display="block">نقطه انتخاب
                                    شد. نظر خود را بنویسید:</Typography>
                                <Stack direction="row" spacing={1}>
                                    <TextField fullWidth size="small" placeholder="نوشتن نظر..." value={newCommentText}
                                               onChange={(e) => setNewCommentText(e.target.value)} sx={textFieldSx}/>
                                    <IconButton color="primary" onClick={handleAddComment}
                                                disabled={!newCommentText}><SendIcon/></IconButton>
                                </Stack>
                            </Box>
                        ) : isImage(previewFile?.file) && (
                            <Box p={2} textAlign="center" borderTop={`1px solid ${theme.palette.divider}`}>
                                <Typography variant="caption" color="text.secondary">برای افزودن نظر، روی بخشی از تصویر
                                    کلیک کنید.</Typography>
                            </Box>
                        )}
                    </Box>
                </DialogContent>
            ...
            >
                ...
                <DialogContent>
                    <Stack spacing={3} mt={1}>
                        ...
                        <TextField select label="پوشه / دسته‌بندی" value={category}
                                   onChange={(e) => setCategory(e.target.value)} fullWidth sx={textFieldSx}>
                            {FOLDERS.filter(f => f.id !== 'all').map(f => <MenuItem key={f.id}
                                                                                    value={f.id}>{f.label}</MenuItem>)}
                        </TextField>
                        <TextField label="توضیحات" value={description} onChange={(e) => setDescription(e.target.value)}
                ...
                <DialogActions sx={{p: 2}}>
                    ...
                    <Button onClick={handleUpload} variant="contained" color="success"
                            disabled={uploading || !uploadFile} sx={{color: '#fff', fontWeight: 'bold'}}>{uploading ?
                        <CircularProgress size={24}/> : 'آپلود'}</Button>
                </DialogActions>
            </Dialog>
        </Box>
    );
