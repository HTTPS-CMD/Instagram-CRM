...
import {
    Box, Button, Typography, Paper, CircularProgress, Stack, Divider, Chip
} from '@mui/material';
...
import ReactMarkdown from 'react-markdown'; // برای نمایش خوشگل متن (اختیاری، اگر نصب نیست متن ساده نشان می‌دهد)
        ...
        <Box>
            {!analysis && !loading && (
                ...
                >
                    ...
                    <Button
                        ...
                        startIcon={<AIIcon />}
                        sx={{
                            bgcolor: '#00e676', color: '#000', fontWeight: 'bold', px: 4, py: 1.5, fontSize: '1.1rem',
            ...
            {loading && (
                <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center" py={10}>
                    ...
                    <Typography variant="h6" mt={3} color="primary">در حال تفکر و تحلیل داده‌ها...</Typography>
                    <Typography variant="caption" color="text.secondary">لطفاً چند لحظه صبر کنید</Typography>
                </Box>
            )}
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    <Paper sx={{ p: 4, borderRadius: 3, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider' }}>
                        <Stack direction="row" alignItems="center" spacing={1} mb={2}>
                            ...
                            <Typography variant="h6" fontWeight="bold" color="success.main">نتیجه تحلیل هوشمند</Typography>
                        </Stack>
                        <Divider sx={{ mb: 3 }} />

                        <Box sx={{ lineHeight: 2, fontSize: '1.05rem', whiteSpace: 'pre-wrap' }}>
                            {/* اگر react-markdown نصب باشد بهتر است، اگر نه متن ساده */}
                            {analysis}
                        </Box>

                        <Box mt={4} display="flex" justifyContent="flex-end">
                            <Button variant="outlined" onClick={() => setAnalysis(null)} startIcon={<BoltIcon />}>
                                تحلیل مجدد
                            </Button>
                        </Box>
                    </Paper>
