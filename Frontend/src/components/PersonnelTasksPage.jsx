...
import {
    ...
    MenuItem, Stack, CircularProgress, Tooltip, Checkbox, useTheme, alpha
} from '@mui/material';
import {
    Add as AddIcon,
...
import { getAllTasks, createPersonnelTask, updatePersonnelTask, deletePersonnelTask, getUsers, getProjects } from '../api';
...
function PersonnelTasksPage() {
    ...
    return (
        <Box>
                    ...
                    {filteredTasks.length === 0 ? (
                        ...
                        <Grid item xs={12} md={6} lg={4} key={task.id}>
                            <Paper sx={{
                                ...
                                transition: '0.2s', '&:hover': { transform: 'translateY(-3px)', boxShadow: theme.shadows[4] }
                            }}>
                                <Stack direction="row" justifyContent="space-between" alignItems="start" mb={1}>
                                    ...
                                    {task.status === 'done' && <Chip label="انجام شد" size="small" color="success" variant="outlined" />}
                                </Stack>

                                <Typography variant="body2" color="text.secondary" sx={{ ml: 5, mb: 2, whiteSpace: 'pre-wrap' }}>
                                    {task.description || 'بدون توضیحات'}
                                </Typography>

                                <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ borderTop: `1px solid ${theme.palette.divider}`, pt: 2, ml: 1 }}>
                                    <Stack direction="row" spacing={1}>
                                        {task.project_details && (
                                            <Tooltip title={`پروژه: ${task.project_details.project_name}`}>
                                                <Chip label={task.project_details.project_name} size="small" sx={{ fontSize: '0.65rem', maxWidth: 100, bgcolor: alpha(theme.palette.action.active, 0.1) }} />
                                            </Tooltip>
                                        )}
                                        {task.due_date && (
                                            <Chip label={jMoment(task.due_date).format('jYYYY/jMM/jDD')} size="small" variant="outlined" sx={{ fontSize: '0.65rem', color: 'text.secondary', borderColor: theme.palette.divider }} />
                                        )}
                                    </Stack>

                                    <Box>
                                        <IconButton size="small" onClick={() => handleEditOpen(task)} color="info"><EditIcon fontSize="small" /></IconButton>
                                        {isAdmin && (
            ...
            >
                <DialogTitle sx={{borderBottom: `1px solid ${theme.palette.divider}`}}>{editingTask ? 'ویرایش تسک' : 'تعریف تسک جدید'}</DialogTitle>
                <DialogContent>
