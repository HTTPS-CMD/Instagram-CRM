...
import {createServiceRequest, getExtraServices, getProjects} from '../api';
...
function ExtraServicesPage() {
    ...
    const fetchData = async () => {
        try {
            const [projRes, servRes] = await Promise.all([getProjects(), getExtraServices()]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const promises = Object.entries(cart).map(([serviceId, quantity]) => {
                const payload = {
                    project: parseInt(selectedProject), service: parseInt(serviceId), quantity: parseInt(quantity)
                };
    ...
    return (<Box sx={{width: '100%', maxWidth: '1600px', mx: 'auto'}}>
        ...
        <Grid container spacing={3}>
            {services.map(service => (<Grid item xs={12} sm={6} md={4} lg={3} key={service.id}>
                <Card
                    sx={{
                        ...glassCardSx,
                        transition: '0.3s',
                    ...
                    <CardContent sx={{textAlign: 'center'}}>
                        <Box sx={{mb: 2, opacity: 0.9}}>
                            {ICON_MAP[service.icon_name] ||
                                <StarIcon fontSize="large" sx={{color: theme.palette.text.primary}}/>}
                        </Box>
                        <Typography variant="h6" fontWeight="bold" gutterBottom
                                    color="text.primary">{service.title}</Typography>
                        <Typography variant="body2"
                                    sx={{color: theme.palette.text.secondary, height: 40, overflow: 'hidden'}}>
                        ...
                        <Stack direction="row" alignItems="center" justifyContent="center" spacing={2} mt={3}
                               sx={{bgcolor: alpha(theme.palette.action.hover, 0.1), borderRadius: 50, p: 0.5}}>
                            ...
                            <Typography variant="h6" fontWeight="bold"
                                        sx={{minWidth: 20, color: theme.palette.text.primary}}>
                                {cart[service.id] || 0}
                            </Typography>
        ...
        >
            <Box>
                <Typography variant="body2" sx={{color: theme.palette.text.secondary}}>مبلغ قابل
                    پرداخت:</Typography>
                <Typography variant="h4" fontWeight="900" sx={{color: theme.palette.warning.main}}>
                    {formatPrice(calculateTotal())} <span
                    style={{fontSize: '1rem', color: theme.palette.text.primary}}>تومان</span>
                </Typography>
            </Box>
            <Button
                variant="contained"
