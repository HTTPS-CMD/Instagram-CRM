...
import {
    ...
    Avatar, Tooltip, CircularProgress, Alert, useTheme, alpha
} from '@mui/material';
import {
    Add as AddIcon, Edit as EditIcon, Delete as DeleteIcon,
    Phone as PhoneIcon, AttachMoney as MoneyIcon,
    Person as PersonIcon, TrendingUp as StatsIcon,
    CheckCircle as WonIcon, Cancel as LostIcon
} from '@mui/icons-material';
import { useSnackbar } from 'notistack';
import { getLeads, createLead, updateLead, deleteLead, getTargetAudiences } from '../api';
    ...
    return (
        <Box sx={{ height: 'calc(100vh - 100px)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                ...
                <Button variant="contained" startIcon={<AddIcon />} onClick={() => handleOpenModal()} sx={{ bgcolor: 'primary.main', fontWeight:'bold' }}>
                    سرنخ جدید
                </Button>
            </Stack>
                ...
                <DragDropContext onDragEnd={onDragEnd}>
                    <Box sx={{ display: 'flex', overflowX: 'auto', gap: 2, height: '100%', pb: 2 }}>
                        {columnsData.map((column) => (
                            <Box key={column.id} sx={{ minWidth: 280, width: 280, display: 'flex', flexDirection: 'column' }}>
                                <Paper sx={{
                                    p: 2, flexGrow: 1,
                                    ...
                                    <Stack direction="row" justifyContent="space-between" mb={2}>
                                        <Typography fontWeight="bold" variant="subtitle2" color="text.primary">{column.title}</Typography>
                                                ...
                                                {column.items.map((lead, index) => (
                                                    <Draggable key={lead.id} draggableId={lead.id.toString()} index={index}>
                                                        {(provided, snapshot) => (
                                                            ...
                                                            >
                                                                <Typography fontWeight="bold" mb={0.5}>{lead.title}</Typography>
                                                                ...
                                                                {lead.target_audience_name && (
                                                                    <Chip
                                                                        ...
                                                                        variant="outlined"
                                                                        sx={{
                                                                            height: 20, fontSize: '0.65rem', mb: 1,
                                                                            color: 'text.secondary', borderColor: theme.palette.divider
                                                                        }}
                                                                    />
                                                                )}
                                                                ...
                                                                {lead.estimated_value > 0 && (
                                                                    <Typography variant="caption" color="success.main" display="block" mb={1} fontWeight="bold">
                                                                        💰 {Number(lead.estimated_value).toLocaleString()} تومان
                                                                    </Typography>
                                                                )}

                                                                <Stack direction="row" justifyContent="flex-end" borderTop={`1px solid ${theme.palette.divider}`} pt={1}>
                                                                    <IconButton size="small" onClick={() => handleOpenModal(lead)} color="info"><EditIcon fontSize="small" /></IconButton>
            ...
            >
                <DialogTitle sx={{borderBottom: `1px solid ${theme.palette.divider}`}}>{editingLead ? 'ویرایش سرنخ' : 'افزودن سرنخ جدید'}</DialogTitle>
                <DialogContent>
                    <Stack spacing={2} mt={2}>
                        ...
                        <Stack direction="row" spacing={2}>
                            ...
                            <TextField
                                ...
                                value={formData.estimated_value} onChange={e => setFormData({...formData, estimated_value: e.target.value})}
                                sx={textFieldSx}
                            />
                        </Stack>
