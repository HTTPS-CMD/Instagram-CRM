...
import {
    ...
    MenuItem, Switch, FormControlLabel, Stack, Chip, useTheme, alpha
} from '@mui/material';
import {
    Add as AddIcon,
    Delete as DeleteIcon,
    Bolt as BoltIcon, // آیکون رعد و برق برای اتوماسیون
...
function AutomationPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        action_type: 'create_default_tasks',
        action_data_input: '', // ورودی متن برای دیتا
        is_active: true
    });
    ...
    const toggleRule = async (rule) => {
        try {
            ...
            await apiClient.patch(`/api/v1/automation-rules/${rule.id}/`, { is_active: newVal });
    ...
    return (
        <Box>
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={4}>
                <Typography variant="h5" fontWeight="bold" sx={{ display:'flex', alignItems:'center', gap:1, color: theme.palette.text.primary }}>
                    <BoltIcon sx={{ color: theme.palette.warning.main }} /> مرکز اتوماسیون هوشمند
                </Typography>
                <Button variant="contained" startIcon={<AddIcon />} onClick={() => setOpen(true)} sx={{ bgcolor: 'primary.main', fontWeight:'bold', boxShadow: theme.shadows[3] }}>
                    ساخت ربات جدید
                </Button>
            </Stack>
                    ...
                    <Grid item xs={12} md={6} lg={4} key={rule.id}>
                        <Paper sx={{
                            ...
                            <Box display="flex" justifyContent="space-between" alignItems="start" mb={2}>
                                <Typography variant="h6" fontWeight="bold" color="text.primary">{rule.name}</Typography>
                                <FormControlLabel
                                    control={<Switch checked={rule.is_active} onChange={() => toggleRule(rule)} color="success" size="small" />}
                            ...
                            <Stack spacing={2}>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: theme.palette.info.main }}>
                                    <TriggerIcon fontSize="small" />
                                    <Typography variant="body2" color="text.secondary">
                                        {rule.trigger_type === 'project_created' ? 'وقتی پروژه جدید ساخته شد' : 'وقتی تسکی تمام شد'}
                                    </Typography>
                                </Box>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: theme.palette.success.main }}>
                                ...
                                <Paper sx={{
                                    ...
                                    {rule.action_type === 'create_default_tasks' ?
                                        (rule.action_data.tasks || rule.action_data_input || '---') :
                                        (rule.action_data.message || '---')
                                    }
                                </Paper>
                            </Stack>
                            ...
                            <IconButton
                                ...
                                sx={{ position: 'absolute', bottom: 10, left: 10, opacity: 0.7, '&:hover': { opacity: 1, bgcolor: alpha(theme.palette.error.main, 0.1) } }}
                            >
            ...
            >
                <DialogTitle sx={{ borderBottom: `1px solid ${theme.palette.divider}` }}>تعریف قانون جدید</DialogTitle>
                <DialogContent>
                    <Stack spacing={3} mt={2}>
                        ...
                        <TextField
                            ...
                            sx={inputStyle}
                        >
                            <MenuItem value="create_default_tasks">تسک‌های پیش‌فرض بساز</MenuItem>
                        ...
                        <Button variant="contained" color="primary" onClick={handleSave} size="large" sx={{fontWeight:'bold', py: 1.5}}>
                            ذخیره و فعال‌سازی
                        </Button>
                    </Stack>
                </DialogContent>
