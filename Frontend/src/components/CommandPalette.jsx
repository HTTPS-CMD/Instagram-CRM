    ...
    >
        <Box sx={{p: 2, borderBottom: `1px solid ${theme.palette.divider}`}}>
            <TextField
                ...
                variant="standard"
                InputProps={{
                    disableUnderline: true, startAdornment: (<InputAdornment position="start">
                        <SearchIcon sx={{color: theme.palette.primary.main, fontSize: 28, mr: 1}}/>
                    </InputAdornment>), // ✅ رنگ متن ورودی
        ...
        <DialogContent sx={{p: 0, minHeight: 100, maxHeight: 400}}>
            {results.length > 0 ? (<List>
                {results.map((item, index) => (<ListItemButton
                    key={index}
                    onClick={() => handleSelect(item.link)}
                    sx={{
                        borderBottom: `1px solid ${theme.palette.divider}`, // ✅ استایل هاور داینامیک
                        '&:hover': {
                ...
                >
                    ...
                    <ListItemText
                        primary={<Typography variant="body1" color="text.primary" fontWeight="bold">
                            {item.title}
                        </Typography>}
            ...
            </List>) : query ? (<Box p={4} textAlign="center" color="text.secondary">
                <Typography>موردی یافت نشد.</Typography>
            </Box>) : (<Box p={2} display="flex" justifyContent="space-between" color="text.disabled">
                <Typography variant="caption">دستورات پیشنهادی:</Typography>
                <Typography variant="caption">پروژه، کاربر، سناریو...</Typography>
            </Box>)}
        </DialogContent>

        <Box p={1} bgcolor={alpha(theme.palette.action.active, 0.05)} display="flex" justifyContent="flex-end"
             px={2}>
            <Typography variant="caption" color="text.secondary">
                    <span style={{
                        border: `1px solid ${theme.palette.divider}`, borderRadius: 4, padding: '0 4px', marginRight: 5
                    }}>ESC</span> برای بستن
            </Typography>
        </Box>
    </Dialog>);
