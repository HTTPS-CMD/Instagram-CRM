...
import {createScenario, deleteScenario, generateScenarioAI, getScenarios, updateScenario} from "../api";
import {
    Alert,
    alpha,
    Box,
        ...
        <Box>
            <Box component="form" onSubmit={handleSubmit} sx={{mt: 2}}>
                ...
                <Grid container spacing={3}>
                    <Grid item xs={12}>
                        <TextField name="title" label="عنوان سناریو" value={formData.title} onChange={handleChange}
                                   fullWidth required sx={inputStyle}/>
                    </Grid>
                    <Grid item xs={12}>
                        <TextField name="summary" label="خلاصه" value={formData.summary} onChange={handleChange}
                                   fullWidth multiline rows={2} sx={inputStyle}/>
                    </Grid>
                    <Grid item xs={12} md={4}>
                        <FormControl fullWidth sx={inputStyle}>
                            <InputLabel>سبک</InputLabel>
                            <Select name="style" value={formData.style} label="سبک" onChange={handleChange}
                                    sx={{textAlign: 'right'}}>
                                {SCENARIO_STYLES.map(s => <MenuItem key={s.value} value={s.value}
                                                                    sx={{direction: 'rtl'}}>{s.label}</MenuItem>)}
                            </Select>
                        </FormControl>
                    </Grid>
                    <Grid item xs={12} md={4}>
                        <FormControl fullWidth sx={inputStyle}>
                            ...
                            <Select name="goal" value={formData.goal} label="هدف" onChange={handleChange}
                                    sx={{textAlign: 'right'}}>
                                {SCENARIO_GOALS.map(g => <MenuItem key={g.value} value={g.value}
                                                                   sx={{direction: 'rtl'}}>{g.label}</MenuItem>)}
                            </Select>
                        </FormControl>
                    </Grid>
                    <Grid item xs={12} md={4}>
                        <FormControl fullWidth sx={inputStyle}>
                            ...
                            <Select name="status" value={formData.status} label="وضعیت" onChange={handleChange}
                                    sx={{textAlign: 'right'}}>
                                {SCENARIO_STATUSES.map(s => <MenuItem key={s.value} value={s.value}
                                                                      sx={{direction: 'rtl'}}>{s.label}</MenuItem>)}
                            </Select>
                        </FormControl>
                    </Grid>
                    <Grid item xs={12}>
                        <TextField name="full_scenario_text" label="متن کامل سناریو" value={formData.full_scenario_text}
                                   onChange={handleChange} fullWidth multiline rows={8} required sx={inputStyle}/>
                    </Grid>
                    <Grid item xs={12}>
                        <Stack direction="row" spacing={2} justifyContent="flex-end">
                            <Button onClick={onCancel} color="inherit">انصراف</Button>
                            <Button type="submit" variant="contained" color="primary" disabled={loading}>{loading ?
                                <CircularProgress size={24}/> : 'ذخیره'}</Button>
                        </Stack>
                    </Grid>
                </Grid>
            </Box>
...
function ScenarioList({projectId, isAdmin}) {
    ...
    const getStatusChip = (status) => {
        ...
        return <Chip label={statusInfo.label} color={colorMap[statusInfo.value] || 'default'} size="small"
                     variant={status === 'idea' ? 'filled' : 'outlined'}/>;
    };
        ...
        <Box>
            <Stack direction="row" spacing={2} mb={4}>
                ...
                <Paper onClick={() => setCurrentType('story')} sx={typeCardStyle('story')} elevation={0}>
                    <StoryIcon
                        sx={{fontSize: 30, color: currentType === 'story' ? theme.palette.warning.main : 'inherit'}}/>
                    <Box>
            ...
            <Paper elevation={3}
                   sx={{p: 0, borderRadius: 2, overflow: 'hidden', bgcolor: theme.palette.background.paper}}>
                <List sx={{p: 0}}>
                    {filteredScenarios.length === 0 ? (
                        <Box p={4} textAlign="center">
                            <Typography color="text.secondary">هنوز سناریویی در
                                بخش {currentType === 'reels' ? 'ریلز' : 'استوری'} ثبت نشده است.</Typography>
                        </Box>
                    ) : (
                                ...
                                <Stack direction="row" spacing={1} alignItems="center" justifyContent="flex-end">
                                    {scenario.final_file && (
                                        <Tooltip title="دانلود فایل نهایی">
                                            <IconButton component="a" href={scenario.final_file} target="_blank"
                                    ...
                                    {canApprove && scenario.status === 'idea' && (
                                        <>
                                            <Tooltip title="تایید"><IconButton size="small"
                                                                               onClick={() => handleClientAction(scenario, 'approve')}
                                                                               sx={{
                                            ...
                                            <Tooltip title="اصلاح"><IconButton size="small"
                                                                               onClick={() => handleClientAction(scenario, 'reject')}
                                                                               sx={{
                                    ...
                                    {isAdmin && (
                                        <>
                                            <IconButton onClick={() => handleOpenFormModal(scenario)}
                    ...
                    <Stack spacing={3}>
                        ...
                        <Paper elevation={0} sx={{
                            ...
                            <Stack direction="row" spacing={2} flexWrap="wrap" alignItems="center">
                                <Chip icon={<FiberManualRecordIcon/>} label={`سبک: ${selectedScenario?.style}`}
                                      size="small"/>
                                <Chip icon={<FiberManualRecordIcon/>} label={`هدف: ${selectedScenario?.goal}`}
                                      size="small"/>
                                <Chip icon={<FiberManualRecordIcon/>}
                                      label={`نوع: ${selectedScenario?.scenario_type === 'story' ? 'استوری' : 'ریلز'}`}
                                ...
                                {canApprove && selectedScenario?.status === 'idea' && (
                                    <Stack direction="row" spacing={1} ml="auto">
                                        <Button variant="contained" color="success" size="small"
                                                startIcon={<CheckIcon/>}
                                                onClick={() => handleClientAction(selectedScenario, 'approve')}>تایید</Button>
                                        <Button variant="outlined" color="error" size="small" startIcon={<CancelIcon/>}
                                                onClick={() => handleClientAction(selectedScenario, 'reject')}>اصلاح</Button>
                                    </Stack>
                                )}
                            </Stack>
                        </Paper>
                        ...
                        <Box>
                            <Typography variant="subtitle1" fontWeight="bold" gutterBottom color="primary">متن کامل
                                سناریو:</Typography>
                            <Paper elevation={1} sx={{
                                ...
                                <Typography variant="body1"
                                            sx={{whiteSpace: 'pre-wrap'}}>{selectedScenario?.full_scenario_text || '---'}</Typography>
                            </Paper>
                        </Box>
                        ...
                        {selectedScenario && user &&
                            <ScenarioComments scenarioId={selectedScenario.id} currentUser={user}/>}
