...
import {
    ...
    FormControl, InputLabel, Tooltip, Avatar, CircularProgress, Stack, Chip, useTheme, alpha
} from '@mui/material';
import {
...
function GanttPage() {
    ...
    const calculateDateRange = (data) => {
        ...
        setDateRange({
            start: minDate.clone().subtract(5, 'days'),
            end: maxDate.clone().add(10, 'days')
        });
    };
    ...
    const getTaskStyle = (task) => {
        ...
        return {
            left: diffDaysStart * cellWidth,
            width: Math.max(durationDays * cellWidth, cellWidth), // حداقل عرض یک سلول
        ...
        <Box sx={{ height: 'calc(100vh - 100px)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <Paper sx={{
                ...
                <Stack direction="row" alignItems="center" spacing={2}>
                    ...
                    <Chip label={`${ganttData.length} پروژه فعال`} size="small" sx={{ bgcolor: alpha(theme.palette.action.active, 0.1), color: 'text.primary' }} />
                </Stack>
            ...
            <Box sx={{
                ...
                <Box sx={{
                    width: SIDEBAR_WIDTH, flexShrink: 0, borderRight: `1px solid ${theme.palette.divider}`,
                    bgcolor: alpha(theme.palette.background.paper, 0.5), overflowY: 'hidden', pt: `${HEADER_HEIGHT}px` // فضای خالی برای هدر
                }}>
                        ...
                        {ganttData.map(project => (
                            <Box key={project.id} mb={1}>
                                ...
                                {project.tasks.map(task => (
                                    <Box key={task.id} sx={{
                                        p: 1, pl: 4, height: 40, display: 'flex', alignItems: 'center',
                ...
                <Box sx={{ flexGrow: 1, overflowX: 'auto', position: 'relative' }}>
                    <Box sx={{ minWidth: totalWidth, position: 'relative' }}>
                        ...
                        {/* Header (Dates) */}
                        <Box sx={{
                            height: HEADER_HEIGHT, position: 'sticky', top: 0, zIndex: 10,
                            ...
                            {timeAxis.map((day, index) => {
                                ...
                                return (
                                    <Box key={index} sx={{
                                        width: cellWidth, minWidth: cellWidth, borderRight: `1px solid ${theme.palette.divider}`,
                                        textAlign: 'center', pb: 1, position: 'relative'
                                    }}>
                                        {/* نمایش ماه (فقط در شروع ماه) */}
                                        {isMonthStart && (
                                            <Typography variant="caption" sx={{ position: 'absolute', top: 5, left: 5, fontWeight: 'bold', color: theme.palette.primary.main, whiteSpace:'nowrap' }}>
                        ...
                        <Box sx={{ position: 'relative' }}>
                            ...
                            <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', pointerEvents: 'none', zIndex: 0 }}>
                                {timeAxis.map((_, i) => (
                                    <Box key={i} sx={{ width: cellWidth, minWidth: cellWidth, borderRight: `1px dashed ${alpha(theme.palette.divider, 0.3)}` }} />
                                ))}
                            </Box>
                            ...
                            <Box sx={{ pt: 0 }}>
                                {ganttData.map(project => (
                                    <Box key={project.id} mb={1}>
                                        ...
                                        <Box sx={{ height: 42, borderBottom: `1px solid ${theme.palette.divider}`, bgcolor: alpha(theme.palette.action.hover, 0.05) }} />

                                        {/* ردیف تسک‌ها */}
                                        {project.tasks.map(task => {
                                            const style = getTaskStyle(task);
                                            return (
                                                <Box key={task.id} sx={{ height: 40, position: 'relative', borderBottom: `1px solid ${alpha(theme.palette.divider, 0.5)}` }}>
                                                    <Tooltip
                                                    ...
                                                    >
                                                        <Box sx={{
                                                            ...
                                                            {task.assignee_avatar && (
                                                                <Avatar src={task.assignee_avatar} sx={{ width: 18, height: 18, mr: 0.5, border: '1px solid white' }} />
                                                            )}
                                                            <Typography variant="caption" color="white" noWrap fontWeight="bold" sx={{ fontSize: '0.7rem', textShadow: '0 1px 2px rgba(0,0,0,0.5)' }}>
                                                                {task.title}
                                                            </Typography>
                                                        </Box>
                                                    </Tooltip>
                        ...
                        <Box sx={{
                            ...
                            top: 0, bottom: 0,
                            width: 2, bgcolor: theme.palette.error.main,
                            zIndex: 5,
                            pointerEvents: 'none'
                        }}>
                            <Box sx={{ position: 'absolute', top: -10, left: -24, bgcolor: theme.palette.error.main, color: 'white', px: 0.5, borderRadius: 1, fontSize: '0.6rem' }}>
                                امروز
                            </Box>
                        </Box>
